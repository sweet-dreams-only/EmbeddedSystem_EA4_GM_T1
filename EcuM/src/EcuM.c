...
#if (  (ECUM_SW_MAJOR_VERSION != (0x07u)) \
...
/* vendor specific version information is BCD coded */
#if (  (ECUM_CBK_SW_MAJOR_VERSION != (0x07u)) \
...
# endif
  {
    ...
    {
      /* ----- Implementation ----------------------------------------------- */
      /* Store the passed EcuM State. */
      EcuM_SetBswMRequestedModuleState(state);

      /* Notify the Rte about the mode switch.
       *  */
      ECUM_RTE_SWITCH(mode);
    }
  }
...
# endif
      ...
      {
        VAR(uint8, AUTOMATIC) resetReason;
...
# endif /* ((ECUM_CONFIGURATION_VARIANT == ECUM_CONFIGURATION_VARIANT_POSTBUILD_LOADABLE) || (ECUM_POSTBUILD_VARIANT_SUPPORT == STD_ON)) */
    ...
    /* #51 Start the AUTOSAR OS with the default appMode. */
...
FUNC(void, ECUM_CODE) EcuM_ValidateWakeupEvent(VAR(EcuM_WakeupSourceType, AUTOMATIC) WakeupSource)
{
  ...
  else
  {
     ...
     * - Always if the WakeupSource is a ComChannel.
...
#endif
    {
      /* #20 In the case of defensive behavior: Check if the passed caller is allowed to shut down the ECU. */
