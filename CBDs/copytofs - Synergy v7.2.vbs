...
'
'  Version Control:
'  Date Created:      Wed Mar  7 10:00:00 2012
'  %version:          1 %
'  %derived_by:       nzx5jd %
'  %date_modified:    Wed Nov 13 14:31:00 2012 %
' ----------------------------------------------------------------------------------------------------------------------
'   Date      Rev      Author    Change Description                                                              SCR #
'   --------  -------  --------  ------------------------------------------------------------------------------  -----
'   07/29/13  1        OT        Initial Version
' ----------------------------------------------------------------------------------------------------------------------
...
'sDatabasePath =		"\\usmisag-ap58\ccmdb72\ses_dev_ea4"	' Database Path for the synergy connection
sDatabasePath =		"\\usmisag-ap58\ccmdb72\ses_dev_ea4"	' Database Path for the synergy connection
'sEngineHost =   	"http://usmisag-ap58:8400"							' Engine Host for the synergy connection
sEngineHost =   	"usmisag-ap58"							' Engine Host for the synergy connection
...
If bFound = False Then
	...
	If oFSO.FolderExists("C:\Program Files") Then
		If oFSO.FolderExists("C:\Program Files\Telelogic") Then
			sCCMsearch = SearchFolder("C:\Program Files\Telelogic", sCCMexe, False)
			If sCCMsearch <> "" Then
				bFound = True
				sCCMexe = """" & sCCMsearch & """"
			End If
...
If bFound = False Then
	...
	If oFSO.FolderExists("C:\Program Files (x86)") Then
		If oFSO.FolderExists("C:\Program Files (x86)\Telelogic.lonnieremove") Then
			sCCMsearch = SearchFolder("C:\Program Files (x86)\Telelogic", sCCMexe, False)
			If sCCMsearch <> "" Then
				bFound = True
				sCCMexe = """" & sCCMsearch & """"
			End If
...
If Not oFSO.FolderExists(sWorkingArea) Then
	If bAutoMode = True Then
		sMsg = "Error - unable to find the local database folder: " & vbCrLf & "   " & sWorkingArea & vbCrLf &_
			   "Update the value of the sWorkingArea parameter in copytofs.vbs" & vbCrLf & sStdErr
		Message sMsg
		WScript.Quit 6
	End If
	
	sMsg = "Select the local database folder for Synergy" & vbCrLf & "(C:\ccm_wa by default)"
	Set oFolder = oShlApp.BrowseForFolder(0, sMsg, 1, 17)
	If oFolder Is Nothing Then
		Disconnect
		WScript.Quit 0
	End If
	sWorkingArea = oFolder.Self.Path
End If

ErrorCheck "find local DB directory"
...
Else
	...
	sMsg = "Enter the integration project name" & vbCrLf & vbCrLf & _
		   "  Format:    Project_Name-version" & vbCrLf & _
		   "      Or         Project_Name-version:project:instance"
...
bCopyParent = False
If InStr(1, sProjectNameShort, ".") Then
	If bAutoMode = True Then
		SubProjects(0) = sProjectNameShort
		bCopyParent = True
	Else
		sMsg = "Copy parent project to file system?" & vbCrLf &_
			   "(" & sProjectNameShort & ")" & vbCrLf &_
			   "Note: working projects cannot be copied"
