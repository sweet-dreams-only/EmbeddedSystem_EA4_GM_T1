 ...
 *********************************************************************************************************************/
	...
	VAR(uint8, AUTOMATIC) SteerSysPrfmncModReqRaw_Cnt_T_u08;
	...
	/* Setup default values for outputs */
	DesRtIdx_Cnt_T_u08 = *Rte_Pim_GmMsg232BusHiSpdPrevDesRtIdx();


	/* Read inputs */
	(void)Rte_Read_LstRxnTiMsg232_Val(&LstRxnTi_100uS_T_u32);
	(void)Rte_Read_SteerSysPrfmncModReqRaw_Val(&SteerSysPrfmncModReqRaw_Cnt_T_u08);
	...
	if (ElpdTi_mS_T_u32 > Rte_Prm_GmMsg232BusHiSpdMissTiOut_Val())
	{
		/* Return to default "normal" mode when message is mising */
		DesRtIdx_Cnt_T_u08 = 0U;
		(void)Rte_Call_SetNtcSts_Oper(NTCNR_0X141, 1U, NTCSTS_FAILD, 0U);
	}
	else
	{
		(void)Rte_Call_SetNtcSts_Oper(NTCNR_0X141, 1U, NTCSTS_PASSD, 0U);

		/* Only update the desired personality when the received value is in range */
		if ((SteerSysPrfmncModReqRaw_Cnt_T_u08 >= 1U) &&
			(SteerSysPrfmncModReqRaw_Cnt_T_u08 <= 5U))
		{
			DesRtIdx_Cnt_T_u08 = SteerSysPrfmncModReqRaw_Cnt_T_u08 - 1U;
		}
	}


	/* Update the "previous" value for next loop */
	*Rte_Pim_GmMsg232BusHiSpdPrevDesRtIdx() = DesRtIdx_Cnt_T_u08;
