...
* Module Description: Implementation of Handwheel Angle Correlation FDD ES239A
* Project           : CBD 
...
* -------   -------  --------  ---------------------------------------------------------------------------     --------
* 07/24/15  1        SV        Initial Version                                                                 EA4#1100       
* 10/15/15  2        NS        Added stall count rollover protection                                           EA4#2024     
* 12/11/15  3        Selva      Anomoly 3047,1982   fixed                                                    EA4#3073&EA4#3124                                                 
**********************************************************************************************************************/
...
#define CORRLNSTSMAXVAL_CNT_U08     3U

#define AANDBUSABLE_CNT_U08			3U
#define ONLYBUSABLE_CNT_U08			2U
#define ONLYAUSABLE_CNT_U08			1U
#define NONEUSABLE_CNT_U08			0U
#define BOTHVALID_CNT_U08			2U
#define ONEISVALID_CNT_U08			1U
#define NONEVALID_CNT_U08			0U

static FUNC(boolean, HwAgCorrln_CODE) HwAgSigAvlChk(uint8 SigRollg_Cnt_T_u08,
                                                    SigQlfr1 SigQlfr_Cnt_T_enum,
                                                    P2VAR(uint8, AUTOMATIC, HwAgCorrln_VAR) LstRollg_Cnt_T_u08,
                                                    P2VAR(uint8, AUTOMATIC, HwAgCorrln_VAR) LstStall_Cnt_T_u08);
 ...
 *********************************************************************************************************************/
    ...
    VAR(boolean, AUTOMATIC) ImmedCorrlnOk_Cnt_T_lgc = FALSE;
    VAR(SigQlfr1, AUTOMATIC) LongTermCorrlnOk_Cnt_T_enum;
    ...
    (void)Rte_Read_HwAgBARollgCntr_Val(&HwAgBRollgCntr_Cnt_T_u08);
    (void)Rte_Read_HwAgBQlfr_Val(&HwAgBQlfr_Cnt_T_enum);
     
    /****** Start of 'Corrln Signal Availability' ******/
    /* HwAg A Availability Check */
    HwAgAAvl_Cnt_T_lgc = HwAgSigAvlChk(HwAgARollgCntr_Cnt_T_u08,        /* REQ: ES239A #35 I */
                                       HwAgAQlfr_Cnt_T_enum,
                                       Rte_Pim_HwAgARollgCntPrev(),
                                       Rte_Pim_HwAgAStallCntPrev());
    *Rte_Pim_dHwAgCorrlnAgAAvl() = HwAgAAvl_Cnt_T_lgc;                  /* REQ: ES239A #140 I */

    /* HwAg B Availability Check */
    HwAgBAvl_Cnt_T_lgc = HwAgSigAvlChk(HwAgBRollgCntr_Cnt_T_u08,        /* REQ: ES239A #69 I */
                                       HwAgBQlfr_Cnt_T_enum,
                                       Rte_Pim_HwAgBRollgCntPrev(),
                                       Rte_Pim_HwAgBStallCntPrev());
    *Rte_Pim_dHwAgCorrlnAgBAvl() = HwAgBAvl_Cnt_T_lgc;                  /* REQ: ES239A #140 I */
    
    /****** End of 'Corrln Signal Availability' ******/
    
    /****** Start of 'Correlation ' ******/
    
    /* Hand wheel Angle Correlation Diagnostics */
    if((HwAgAAvl_Cnt_T_lgc == TRUE) && (HwAgBAvl_Cnt_T_lgc == TRUE))    /* REQ: ES239A #89 I */
    {
        /* REQ: ES239A #91 I */
        if((Abslt_f32_f32(HwAgA_HwDeg_T_f32 - HwAgB_HwDeg_T_f32)) <= Rte_Prm_HwAgCorrlnHwAgAbsltDif_Val())
        {
            ImmedCorrlnOk_Cnt_T_lgc = TRUE;
            (void)Rte_Call_SetNtcSts_Oper(NTCNR_0X092, 0U, NTCSTS_PREPASSD, Rte_Prm_HwAgCorrlnNtc0x092PassStep_Val());  /* ENDREQ: ES239A #94 */
        }
        else
        {
            ImmedCorrlnOk_Cnt_T_lgc = FALSE;
            (void)Rte_Call_SetNtcSts_Oper(NTCNR_0X092, 0U, NTCSTS_PREFAILD, Rte_Prm_HwAgCorrlnNtc0x092FailStep_Val());
        }
        /* ENDREQ: ES239A #91 */
    }   
    ...
    /* REQ: ES239A #216 I */
    /* REQ: ES239A #194 I */
    (void)Rte_Call_GetNtcQlfrSts_Oper(NTCNR_0X092, &LongTermCorrlnOk_Cnt_T_enum);
    ...
    {
        if (LongTermCorrlnOk_Cnt_T_enum < SIGQLFR_FAILD) 
        {
            if (ImmedCorrlnOk_Cnt_T_lgc == TRUE)
            {
                HwAgCorrlnSt_Cnt_T_u08 = AANDBUSABLE_CNT_U08;    /* CorrlnSt - Action 1 */
            }
            else
            {
                HwAgCorrlnSt_Cnt_T_u08 = NONEUSABLE_CNT_U08;    /* CorrlnSt - Action 4 */
            }
            HwAgIdptSig_Cnt_T_u08 = BOTHVALID_CNT_U08;     /* IdptSig - Action 1 */    
        }
        else
        {
            HwAgCorrlnSt_Cnt_T_u08 = NONEUSABLE_CNT_U08;    /* CorrlnSt - Action 4 */
            HwAgIdptSig_Cnt_T_u08 = NONEVALID_CNT_U08;     /* IdptSig - Action 3 */
        }
    }
    else if (HwAgAAvl_Cnt_T_lgc == TRUE)
    {
        HwAgCorrlnSt_Cnt_T_u08 = ONLYAUSABLE_CNT_U08;    /* CorrlnSt - Action 3 */
        HwAgIdptSig_Cnt_T_u08 = ONEISVALID_CNT_U08;     /* IdptSig - Action 2 */
    }
    else if (HwAgBAvl_Cnt_T_lgc == TRUE)
    {
        HwAgCorrlnSt_Cnt_T_u08 = ONLYBUSABLE_CNT_U08;    /* CorrlnSt - Action 2 */
        HwAgIdptSig_Cnt_T_u08 = ONEISVALID_CNT_U08;     /* IdptSig - Action 2 */
    }
    else
    {
    ...
    HwAgCorrlnSt_Cnt_T_u08 = Lim_u08(HwAgCorrlnSt_Cnt_T_u08,CORRLNSTSMINVAL_CNT_U08,CORRLNSTSMAXVAL_CNT_U08);
    HwAgIdptSig_Cnt_T_u08 = Lim_u08(HwAgIdptSig_Cnt_T_u08,NRVLDSIGMIN_CNT_U08,NRVLDSIGMAX_CNT_U08);
 ...
 *********************************************************************************************************************/
 /*****************************************************************************************************************
  * Name        :   HwAgSigAvlChk
  * Description :   Implementation of 'HwAgAAvlChk' and 'HwAgBAvlChk' blocks.
  *                 Checks Signal Availability.
  * Inputs      :   SigRollg_Cnt_T_u08 - Hand Wheel Angle Rolling Counter
  *                 SigQlfr_Cnt_T_enum - Hand Wheel Angle Qualifier
  *                 *LstRollg_Cnt_T_u08 - Previous Hand Wheel Angle Rolling Counter
  *                 *LstStall_Cnt_T_u08 - Hand Wheel Angle Stall Counter (previous Signal Availability Status)
  * Returns     :   SigAvl_Cnt_T_lgc - Hand Wheel Angle Signal Availability Status
  * Usage Notes :   None
  ****************************************************************************************************************/
static FUNC(boolean, HwAgCorrln_CODE) HwAgSigAvlChk(uint8 SigRollg_Cnt_T_u08,
                                                    SigQlfr1 SigQlfr_Cnt_T_enum,
                                                    ...
                                                    P2VAR(uint8, AUTOMATIC, HwAgCorrln_VAR) LstStall_Cnt_T_u08)
{
    ...
    {
        /* Rollover protection */
        if(*LstStall_Cnt_T_u08 == Rte_Prm_HwAgCorrlnHwAgMaxStall_Val())
        {
            *LstStall_Cnt_T_u08 = Rte_Prm_HwAgCorrlnHwAgMaxStall_Val();
        }
        else
        {
            *LstStall_Cnt_T_u08 = *LstStall_Cnt_T_u08 + 1U;
        }
    }
    else
    {
        *LstStall_Cnt_T_u08 = 0U;
    }
    ...
    *LstRollg_Cnt_T_u08 = SigRollg_Cnt_T_u08;
