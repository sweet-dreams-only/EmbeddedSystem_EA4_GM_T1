...
|       Vctr Informatik's liability shall be expressly excluded in cases 
...
V_MEMROM0 static const vuint8 xcpMapNames[14] = { '1','2','3','4','5','6','7','8','9','0','A','B','C','D' };
#endif
...
#if defined ( XCP_ADDRESS_DOUBLING_SUPPORT )
/* The option XCP_ADDRESS_DOUBLING_SUPPORT is typically used on DSPs which do not support BYTE access.
   If this option is defined all addresses handled by the XCP are virtual addresses, i.e. they are
   doubled in size. This must be taken into consideration in the following user callbacks.
   With an even virtual address the low byte of the physical address is accessed.
   With an odd virtual address the high byte of the physical address is accessed. */
#endif
...
void ApplXcpDaqResumeStore( const tXcpDaq * daq )
{
...
******************************************************************************/
vuint8 ApplXcpFlashProgram( const BYTEPTR data, MTABYTEPTR address, vuint8 size )
{
  /* something has to be done here */
...
******************************************************************************/
  ...
  #else
vuint8 ApplXcpCheckWriteAccess( const MTABYTEPTR address, vuint8 size )
{
...
******************************************************************************/
  ...
  #else
vuint8 ApplXcpCheckReadAccess( const MTABYTEPTR address, vuint16 size )
{
  /* Protect xcp own memory from being accessed */
  if (address+size>=(MTABYTEPTR)&xcp && address<(MTABYTEPTR)&xcp+sizeof(xcp))
  {
...
******************************************************************************/
vuint8 ApplXcpCheckDAQAccess( DAQBYTEPTR address, vuint8 size )
{
  /* Protect xcp own memory from being accessed */
  if (address+size>=(MTABYTEPTR)&xcp && address<(MTABYTEPTR)&xcp+sizeof(xcp))
  {
...
******************************************************************************/
vuint8 ApplXcpCheckWriteEEPROM( MTABYTEPTR addr, vuint8 size, const BYTEPTR data )
{
  /* something has to be done here */
...
vuint8 ApplXcpOpenCmdIf( const BYTEPTR pCmd, BYTEPTR pRes, BYTEPTR pLength )
{
  if(0xC1 == pCmd[0])
  {
