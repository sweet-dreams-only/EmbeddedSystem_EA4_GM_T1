 ...
 * \details    Checks if given chunk's payload size equals the configured payload size
 * \param[in]  chunk
 * \param[in]  config_pt
 * \return     TRUE: Chunk's payload size is as configured
 * \return     FALSE: Chunk's payload size is not as configured
 * \pre        chunk not null, config_pt not null
 * \context    TASK
 * \reentrant  FALSE
 */
 ...
 */
FEE_LOCAL_FUNC(void) Fee_ActInitLinkTableAllocationFsm(void);
...
/**********************************************************************************************************************
 * Fee_InternalInitChunkAllocCopyFsm
 *********************************************************************************************************************/
/*!
 * \brief      Initiates data chunk allocation for copy purpose in newer sector
 * \details    Initiates data chunk allocation for copy purpose in newer sector
 * \param[in,out]  g
 * \pre        linktable chunk has been allocated before, if necessary, target chunk info and source chunk info are already
 *             set accordingly
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalInitChunkAllocCopyFsm(Fee_GlobalsPtrType g);
...
static CONST(struct Fee_StateDescrStruct, FEE_PRIVATE_CONST) Fee_StateDescrTable_at[] =
{
    ...
    /* FEE_STATE_SECTOR_SWITCH_SEARCH_UPWARDS */
    {
        ...
        {
            FEE_STATE_ELSE {FEE_ACT_ID_INIT_LINKTABLE_ALLOCATION, FEE_ACT_ID_NOP},
...
FEE_LOCAL_FUNC(void) Fee_ActInitLinkTableAllocationFsm(void)
{
    ...
    uint8 datasets =  (Fee_Globals_t.CurrentProcessData.BlockId.DataIndex);
    if(Fee_Globals_t.CurrentProcessData.BlockConfig_pt == NULL_PTR)
    {
        datasets++;
    }
    else
    {
        datasets = (uint8)(Fee_Globals_t.CurrentProcessData.BlockConfig_pt->NumberOfDatasets_u8);
    }
...
#if(FEE_GET_WRITE_CYCLE_API == STD_ON)
        context_pt->wccChunk_pt = &Fee_Globals_t.CurrentDataset_t; /* SBSW_FEE_Func_passParamByReference */
#endif
        {
...
FEE_LOCAL_FUNC(void) Fee_InternalInitChunkAllocCopyFsm(Fee_GlobalsPtrType g)
{
    ...
    const Fee_BlockConfigRefType blockConfig = g->CurrentProcessData.BlockConfig_pt;
    uint16 payload;
    ...
    {
        payload = Fee_ChunkInfo_getPayload(&g->CurrentDataset_t.chunk_t); /* SBSW_FEE_Func_passParamByReference */
        /*In FBL mode, during sector switch always chunks with one instance are created*/
        instanceExponent = 1u;
    }

    Fee_Proc_CreateChunkAllocContext(context_pt, &g->SectorSwitchTarget_t, g->CurrentProcessData.BlockId, blockConfig); /* SBSW_FEE_Func_passParamByReference */ /* SBSW_FEE_InitializedProcessData */

#if(FEE_GET_WRITE_CYCLE_API == STD_ON)
    context_pt->wccChunk_pt = &g->CurrentDataset_t; /* SBSW_FEE_ChunkAllocContext */
#endif
    {
        struct Fee_ChunkInfoStruct c;
...
FEE_INLINE_FUNC(void) Fee_InternalDoCopyInit(Fee_GlobalsPtrType g)
{
    ...
    /* PRQA S 3344 1 */ /* MD_FEE_13.2_boolean */
    if(Fee_ChunkInfo_isDataChunk(&g->CurrentDataset_t.chunk_t)) /* SBSW_FEE_Func_passParamByReference */
    {
        ...
        const uint16 configPayload = (uint16)((g->CurrentProcessData.BlockConfig_pt != NULL_PTR) ?
                g->CurrentProcessData.BlockConfig_pt->BlockPayloadSize_u16 :
                                                        Fee_ChunkInfo_getPayload(&source->chunk_t)); /* SBSW_FEE_Func_passParamByReference */
        const uint16_least targetInstances = Fee_ChunkInfo_GetInstanceCount(&target->chunk_t);       /* SBSW_FEE_Func_passParamByReference */
        Fee_Bool chunkNecessary;

        if(configPayload != Fee_ChunkInfo_getPayload(&source->chunk_t)) /* SBSW_FEE_Func_passParamByReference */
        {
        ...
        /* PRQA S 3415 3 */ /* MD_FEE_12.4_queries */
        chunkNecessary = (((source->instance_t.status_t == FEE_INSTANCE_STATUS_OK)
                                    || (source->instance_t.status_t == FEE_INSTANCE_STATUS_INCONSISTENT))
                         && ((!Fee_ChunkInfo_isDataChunk(&target->chunk_t)) /* SBSW_FEE_Func_passParamByReference */
                                    || (target->instance_t.index_t >= targetInstances)
                                    || (configPayload != Fee_ChunkInfo_getPayload(&target->chunk_t)))); /* SBSW_FEE_Func_passParamByReference */
        ...
        {
            Fee_FsmInitSubFsm(&g->FsmStack_t, FEE_STATE_COPY_ALLOCATE_CHUNK); /* SBSW_FEE_Func_passParamByReference */
            Fee_InternalInitChunkAllocCopyFsm(g); /* SBSW_FEE_PointerWrite_refParam */
        }
        else
        {
            Fee_FsmInitSubFsm(&g->FsmStack_t, FEE_STATE_COPY_INSTANCE_COPY); /* SBSW_FEE_Func_passParamByReference */
            Fee_ActInitializeBlockCopy();
        }
    }
    else
