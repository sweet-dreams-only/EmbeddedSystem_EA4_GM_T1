 ...
 *
 *   Description:  C-Code implementation template for SW-C <RtnPahFwl>
 *********************************************************************************************************************/
...
* Nxtr Confidential
*
* Module File Name  : RtnPahFwl.c
* Module Description: Implementation of Return Path Firewall Algorithm SF036A. 
* Project           : CBD 
...
* -------   -------  --------  ---------------------------------------------------------------------------  ---------
* 02/03/16  1         AK       Initial Version                                                          	  EA4#3629 
* 02/26/16  2         KK       cal RtnPahFwlUpprBndTblY table dimensions correction(Fix for anomaly EA4#3980) EA4#3990 
**********************************************************************************************************************/
...
#define PrmRtnPahFwlVehSpd		(*(const Ary1D_u9p7_8 *)		Rte_Prm_RtnPahFwlVehSpd_Ary1D())
#define PrmRtnPahFwlUpprBndTblX	(*(const Ary1D_s11p4_11 *)		Rte_Prm_RtnPahFwlUpprBndTblX_Ary1D())
 ...
 * Symbol: RtnPahFwlInit1
 *********************************************************************************************************************/
   	(void)Rte_Call_SetNtcSts_Oper (NTCNR_0X0C7,
								   0U,
								   NTCSTS_PASSD,
								   NODEBSTEP_CNT_U16);
 ...
 *********************************************************************************************************************/
	...
	VAR(float32, AUTOMATIC)  	RtnCmdLimd_MotNwtMtr_T_f32;
	VAR(float32, AUTOMATIC)  	RtnPahFwlUpprBnd_MotNwtMtr_T_f32;
	VAR(float32, AUTOMATIC)  	RtnPahFwlLowrBnd_MotNwtMtr_T_f32;
	
	VAR(sint16, AUTOMATIC)  	HwAg_HwDeg_T_s11p4;
	VAR(sint16, AUTOMATIC)  	RtnPahFwlLowrBnd_MotNwtMtr_T_s4p11;
	VAR(sint16, AUTOMATIC)  	RtnPahFwlUpprBnd_MotNwtMtr_T_s4p11;
	...
	(void)Rte_Read_RtnCmd_Val(&RtnCmd_MotNwtMtr_T_f32);
	(void)Rte_Read_RtnCmdDi_Logl(&RtnCmdDi_Cnt_T_logl);
	(void)Rte_Read_VehSpd_Val(&VehSpd_Kph_T_f32);
	
	if( (RtnCmdDi_Cnt_T_logl == TRUE) &&
		(MfgEnaSt_Cnt_T_enum != MFGENAST_PRDNMOD) )
	{
		/*** FwlDi ***/
		RtnCmdLimd_MotNwtMtr_T_f32 = RtnCmd_MotNwtMtr_T_f32;	
	}
	else
	{
        /*** Start of NormFwl Oper ***/
        
		HwAg_HwDeg_T_s11p4					= FloatToFixd_s16_f32(HwAg_HwDeg_T_f32, NXTRFIXDPT_FLOATTOP4_ULS_F32);
        VehSpd_Kph_T_u9p7  					= FloatToFixd_u16_f32(VehSpd_Kph_T_f32, NXTRFIXDPT_FLOATTOP7_ULS_F32);
		
		RtnPahFwlUpprBnd_MotNwtMtr_T_s4p11	= BilnrIntrpnWithRound_s16_s16CmnXs16MplY(VehSpd_Kph_T_u9p7,
																					  HwAg_HwDeg_T_s11p4,
																					  PrmRtnPahFwlVehSpd,
																					  (uint16)TblSize_m(PrmRtnPahFwlVehSpd),
																					  Rte_Prm_RtnPahFwlUpprBndTblX_Ary1D(),
																					  Rte_Prm_RtnPahFwlUpprBndTblY_Ary2D(),
																					  (uint16)TblSize_m(PrmRtnPahFwlUpprBndTblX));
		
		RtnPahFwlUpprBnd_MotNwtMtr_T_f32	= FixdToFloat_f32_s16(RtnPahFwlUpprBnd_MotNwtMtr_T_s4p11, NXTRFIXDPT_P11TOFLOAT_ULS_F32);
																	  
		*Rte_Pim_dRtnPahFwlUpprBnd()		= RtnPahFwlUpprBnd_MotNwtMtr_T_f32;
		
		RtnPahFwlLowrBnd_MotNwtMtr_T_s4p11	= -( BilnrIntrpnWithRound_s16_s16CmnXs16MplY(VehSpd_Kph_T_u9p7,
                                                                                     (-HwAg_HwDeg_T_s11p4),
																					     PrmRtnPahFwlVehSpd,
																					     (uint16)TblSize_m(PrmRtnPahFwlVehSpd),
																					     Rte_Prm_RtnPahFwlUpprBndTblX_Ary1D(),
		...
		RtnPahFwlLowrBnd_MotNwtMtr_T_f32	= FixdToFloat_f32_s16(RtnPahFwlLowrBnd_MotNwtMtr_T_s4p11, NXTRFIXDPT_P11TOFLOAT_ULS_F32);
				
		*Rte_Pim_dRtnPahFwlLowrBnd()		= RtnPahFwlLowrBnd_MotNwtMtr_T_f32;
				
		RtnCmdLimd_MotNwtMtr_T_f32 			= Lim_f32(RtnCmd_MotNwtMtr_T_f32, RtnPahFwlLowrBnd_MotNwtMtr_T_f32, RtnPahFwlUpprBnd_MotNwtMtr_T_f32);	
		
		if(Abslt_f32_f32(RtnCmd_MotNwtMtr_T_f32 - RtnCmdLimd_MotNwtMtr_T_f32) > ARCHGLBPRM_FLOATZEROTHD_ULS_F32)																  
		{
			*Rte_Pim_dRtnPahFwlOverBnd() 	= TRUE;
	...
	(void)Rte_Write_RtnCmdLimd_Val(Lim_f32(RtnCmdLimd_MotNwtMtr_T_f32, SYSGLBPRM_MOTTQCMDLOLIM_MOTNWTMTR_F32, SYSGLBPRM_MOTTQCMDHILIM_MOTNWTMTR_F32));
